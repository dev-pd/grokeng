...
async def create_lead(lead: LeadCreate, db: AsyncSession = Depends(get_db_session)):
    """Create a new lead"""
    try:
        ...
        if existing_lead:
            raise HTTPException(
                status_code=400, detail=f"Lead with email {lead.email} already exists"
            )
...
async def update_lead(
    lead_id: int, lead_update: LeadUpdate, db: AsyncSession = Depends(get_db_session)
):
    """Update a lead"""
    try:
        query = select(Lead).where(Lead.id == lead_id)
...
async def delete_lead(lead_id: int, db: AsyncSession = Depends(get_db_session)):
    """Delete a lead"""
    try:
        query = select(Lead).where(Lead.id == lead_id)
        result = await db.execute(query)
...
async def get_lead_stats(db: AsyncSession = Depends(get_db_session)):
